from __future__ import annotations

import logging
from typing import Any, Iterator, Optional
from urllib.parse import urljoin

import requests
from requests.auth import HTTPBasicAuth

from blesta_sdk.core import BlestaResponse
from blesta_sdk.core.dateutil import _month_boundaries

logger = logging.getLogger(__name__)

DEFAULT_TIMEOUT = 30


class BlestaRequest:
    """
    Blesta API processor for making HTTP requests to the Blesta API.
    """

    def __init__(self, url: str, user: str, key: str, timeout: int = DEFAULT_TIMEOUT):
        """
        Initializes the BlestaRequest instance.

        :param url: The base URL of the Blesta API.
        :param user: The API user.
        :param key: The API key.
        :param timeout: Request timeout in seconds (default 30).
        """
        self.base_url = url.rstrip("/") + "/"
        self.user = user
        self.key = key
        self.timeout = timeout
        self._last_request: Optional[dict[str, Any]] = None
        self.session = requests.Session()
        self.session.auth = HTTPBasicAuth(self.user, self.key)

    def __enter__(self) -> BlestaRequest:
        return self

    def __exit__(self, *args: Any) -> None:
        self.session.close()

    def close(self) -> None:
        """Close the underlying HTTP session."""
        self.session.close()

    def get(
        self, model: str, method: str, args: Optional[dict[str, Any]] = None
    ) -> BlestaResponse:
        """
        Makes a GET request to the Blesta API.

        :param model: The API model (e.g., 'clients').
        :param method: The API method (e.g., 'getList').
        :param args: Optional dictionary of query parameters.
        :return: BlestaResponse object containing the response data.
        """
        return self.submit(model, method, args, "GET")

    def post(
        self, model: str, method: str, args: Optional[dict[str, Any]] = None
    ) -> BlestaResponse:
        """
        Makes a POST request to the Blesta API.

        :param model: The API model (e.g., 'clients').
        :param method: The API method (e.g., 'create').
        :param args: Optional dictionary of data to send in the body of the request.
        :return: BlestaResponse object containing the response data.
        """
        return self.submit(model, method, args, "POST")

    def put(
        self, model: str, method: str, args: Optional[dict[str, Any]] = None
    ) -> BlestaResponse:
        """
        Makes a PUT request to the Blesta API.

        :param model: The API model (e.g., 'clients').
        :param method: The API method (e.g., 'update').
        :param args: Optional dictionary of data to send in the body of the request.
        :return: BlestaResponse object containing the response data.
        """
        return self.submit(model, method, args, "PUT")

    def delete(
        self, model: str, method: str, args: Optional[dict[str, Any]] = None
    ) -> BlestaResponse:
        """
        Makes a DELETE request to the Blesta API.

        :param model: The API model (e.g., 'clients').
        :param method: The API method (e.g., 'delete').
        :param args: Optional dictionary of data to send in the body of the request.
        :return: BlestaResponse object containing the response data.
        """
        return self.submit(model, method, args, "DELETE")

    def submit(
        self,
        model: str,
        method: str,
        args: Optional[dict[str, Any]] = None,
        action: str = "POST",
    ) -> BlestaResponse:
        """
        Submits an HTTP request to the Blesta API.

        :param model: The API model (e.g., 'clients').
        :param method: The API method (e.g., 'getList').
        :param args: Optional dictionary of data or query parameters.
        :param action: The HTTP action to perform ('GET', 'POST', 'PUT', 'DELETE').
        :return: BlestaResponse object containing the response data.
        """
        if args is None:
            args = {}

        url = urljoin(self.base_url, f"{model}/{method}.json")
        self._last_request = {"url": url, "args": args}

        try:
            if action == "GET":
                response = self.session.get(url, params=args, timeout=self.timeout)
            elif action == "POST":
                response = self.session.post(url, json=args, timeout=self.timeout)
            elif action == "PUT":
                response = self.session.put(url, json=args, timeout=self.timeout)
            elif action == "DELETE":
                response = self.session.delete(url, json=args, timeout=self.timeout)
            else:
                raise ValueError("Invalid HTTP action specified.")

            return BlestaResponse(response.text, response.status_code)

        except requests.RequestException as e:
            logger.error(f"Request failed: {e}")
            return BlestaResponse(str(e), 500)

    def get_all_pages(
        self,
        model: str,
        method: str,
        args: Optional[dict[str, Any]] = None,
        start_page: int = 1,
    ) -> Iterator[Any]:
        """
        Generator that auto-paginates through all results.

        Calls the API with page=1, page=2, etc. until an empty response
        is returned. Yields individual items from each page.

        :param model: The API model (e.g., 'invoices').
        :param method: The API method (e.g., 'getList').
        :param args: Optional dictionary of query parameters.
        :param start_page: Page number to start from (default 1).
        :return: Iterator of individual result items.
        """
        if args is None:
            args = {}

        page = start_page
        while True:
            page_args = {**args, "page": page}
            response = self.get(model, method, page_args)

            if response.status_code != 200:
                logger.warning(
                    f"Pagination stopped: HTTP {response.status_code} on page {page}"
                )
                return

            data = response.response
            if not data:
                return

            if isinstance(data, list):
                if len(data) == 0:
                    return
                yield from data
            else:
                yield data
                return

            page += 1

    def get_all(
        self,
        model: str,
        method: str,
        args: Optional[dict[str, Any]] = None,
        start_page: int = 1,
    ) -> list[Any]:
        """
        Fetches all pages of results and returns them as a single list.

        Convenience wrapper around :meth:`get_all_pages`.

        :param model: The API model (e.g., 'invoices').
        :param method: The API method (e.g., 'getList').
        :param args: Optional dictionary of query parameters.
        :param start_page: Page number to start from (default 1).
        :return: List of all result items across all pages.
        """
        return list(self.get_all_pages(model, method, args, start_page))

    def get_report(
        self,
        report_type: str,
        start_date: str,
        end_date: str,
        extra_vars: Optional[dict[str, str]] = None,
    ) -> BlestaResponse:
        """
        Fetches a Blesta report.

        Calls report_manager/fetchAll with the required vars[] parameter
        format that Blesta expects.

        :param report_type: The report type (e.g., 'package_revenue').
        :param start_date: Start date string (e.g., '2025-01-01').
        :param end_date: End date string (e.g., '2025-01-31').
        :param extra_vars: Optional additional vars[] parameters.
        :return: BlestaResponse containing report data (use .csv_data to parse).
        """
        args: dict[str, str] = {
            "type": report_type,
            "vars[start_date]": start_date,
            "vars[end_date]": end_date,
        }
        if extra_vars:
            for key, value in extra_vars.items():
                param_key = key if key.startswith("vars[") else f"vars[{key}]"
                args[param_key] = value

        return self.get("report_manager", "fetchAll", args)

    def get_report_series_pages(
        self,
        report_type: str,
        start_month: str,
        end_month: str,
        extra_vars: Optional[dict[str, str]] = None,
    ) -> Iterator[tuple[str, BlestaResponse]]:
        """
        Generator that fetches a report for each month in a date range.

        Yields (period, response) tuples where period is "YYYY-MM"
        and response is the BlestaResponse from get_report().
        Yields all months, including those with errors.

        :param report_type: The report type (e.g., 'package_revenue').
        :param start_month: Start month as "YYYY-MM" (inclusive).
        :param end_month: End month as "YYYY-MM" (inclusive).
        :param extra_vars: Optional additional vars[] parameters.
        :return: Iterator of (period, BlestaResponse) tuples.
        :raises ValueError: If start_month > end_month or format is invalid.
        """
        boundaries = _month_boundaries(start_month, end_month)
        for first_day, last_day, period in boundaries:
            logger.debug(f"Fetching report {report_type!r} for {period}")
            response = self.get_report(
                report_type, first_day, last_day, extra_vars
            )
            yield (period, response)

    def get_report_series(
        self,
        report_type: str,
        start_month: str,
        end_month: str,
        extra_vars: Optional[dict[str, str]] = None,
    ) -> list[dict[str, str]]:
        """
        Fetches a report for each month in a date range and returns
        all CSV rows as a flat list with period metadata.

        Each row dict has a '_period' key added with the "YYYY-MM" value.
        Months that return errors or non-CSV responses are skipped
        with a warning log.

        :param report_type: The report type (e.g., 'package_revenue').
        :param start_month: Start month as "YYYY-MM" (inclusive).
        :param end_month: End month as "YYYY-MM" (inclusive).
        :param extra_vars: Optional additional vars[] parameters.
        :return: Flat list of row dicts from all months.
        :raises ValueError: If start_month > end_month or format is invalid.
        """
        rows: list[dict[str, str]] = []
        for period, response in self.get_report_series_pages(
            report_type, start_month, end_month, extra_vars
        ):
            if response.status_code != 200:
                logger.warning(
                    f"Report {report_type!r} for {period}: "
                    f"HTTP {response.status_code}, skipping"
                )
                continue
            csv_rows = response.csv_data
            if csv_rows is None:
                logger.warning(
                    f"Report {report_type!r} for {period}: "
                    f"no CSV data in response, skipping"
                )
                continue
            for row in csv_rows:
                row["_period"] = period
            rows.extend(csv_rows)
        return rows

    def get_last_request(self) -> Optional[dict[str, Any]]:
        """
        Returns the details of the last request made.

        :return: Dictionary with URL and args of the last request, or None.
        """
        return self._last_request
